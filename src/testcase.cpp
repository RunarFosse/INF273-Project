...
void InstanceRunner::simulatedAnnealing(std::string instance, int experiments, int iterations, std::default_random_engine& rng) {
    ...
    for (int i = 0; i < experiments; i++) {
        ...
        double temperature_f = 0.1, delta_w = 0.0;
        ...
        for (int w = 0; w < 100; w++) {
            ...
            if (solution.isFeasible() && deltaE < 0) {
                incumbent = solution;
        ...
        double deltaAverage = delta_w / updates;
        double temperature_0 = -deltaAverage / log(explorationProbability), alpha = pow(temperature_f / temperature_0, 1.0 / (iterations - 100));
        ...
        for (int j = 0; j < iterations - 100; j++) {
            ...
            if (solution.isFeasible() && deltaE < 0) {
                ...
                if (random(rng) < pow(exp(1), -deltaE / temperature)) {
                    incumbent = solution;
                }
            }
