...
void InstanceRunner::testAlgorithm(std::function<void(std::string, int, int, std::default_random_engine&)> algorithm, bool shallow, int experiments, int iterations, std::default_random_engine& rng) {
...
void InstanceRunner::blindRandomSearch(std::string instanceName, int experiments, int iterations, std::default_random_engine& rng) {
    ...
    for (int i = 0; i < experiments; i++) {
        // Print progress bar
        Debugger::printProgress("Blind Random Search: " + instanceName, i, experiments);
    ...
    Debugger::printProgress("Blind Random Search: " + instanceName, experiments, experiments);
...
void InstanceRunner::localSearch(std::string instanceName, int experiments, int iterations, std::default_random_engine& rng) {
    ...
    for (int i = 0; i < experiments; i++) {
        // Print progress bar
        Debugger::printProgress("Local Search: " + instanceName, i, experiments);
        ...
        for (int j = 0; j < iterations; j++) {
            ...
            if (solution.isFeasible() && solution.getCost() < bestSolution.getCost()) {
                bestSolution = solution;
                //Debugger::printToTerminal(std::to_string(j) + "!!!\n");
            }
        }
    ...
    // Fill the progress bar
    Debugger::printProgress("Local Search: " + instanceName, experiments, experiments);
